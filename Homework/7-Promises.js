...
The function should accept an array of promises and return a single promise that resolves to an array of resolved 
values or rejects with the reason of the first rejected promise.

Instructions

1. Implement a function called `promiseAll` that takes an array of promises as an argument.
2. The function should return a new promise that resolves when all promises in the input array have resolved, 
   and rejects if any of the promises reject.
3. If all promises resolve, the resolved value of the returned promise should be an array containing the resolved
   values of the input promises, in the same order.
        ...
        array.forEach(promise => {
            promise
                .then(result => {
                    ...
                    if (resolveds.length === array.length) {
                        resolve(resolveds);
                    }
                })
                .catch(error => {
...
3. Each object in the resolved array should have properties `status` and `value` or `reason`. 
The `status` can be either `'fulfilled'` or `'rejected'`, and `value` should hold the resolved value (if fulfilled) or `reason` 
should hold the rejection reason (if rejected).
...
const promiseAllSettled = (array) => {
    return new Promise((resolve) => {
        const settled = [array.length];
        array.forEach((promise, index) => {
            promise
                ...
                .finally(() => {
                    if (settled.length === array.length) {
                        resolve(settled);
                    }
                });
        });
...
function asyncFunction2(data) {
    return Promise.reject(data + " - Result from asyncFunction2");
}
...
const chainPromises = (array) => {
    if (!Array.isArray(array) || array.length < 2) Promise.reject(new Error("Input must be an array of at least 2 functions"));
...
module.exports = {
    task1:{promiseAll, promises},
    task2:{promiseAllSettled, promises2},
    task3:{chainPromises, functionsArray},
    task4:{promisify, callbackStyleFunction}
}
